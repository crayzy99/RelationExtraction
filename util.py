...
def xml2Doc(path):
    ...
    title = Handler.title
    highlights = [h for h in Handler.highlights]
    abstract = Handler.abstract
    text = [t for n, t in Handler.text.items()]
    return title, highlights, abstract, text
...
def readAnn(textfolder = "data/dev/"):
    ...
    for f in flist:
        if not f.endswith('.ann'):
            continue
        f_anno = open(os.path.join(textfolder, f), "rU", encoding='utf-8')
        f_text = open(os.path.join(textfolder, f.replace(".ann", ".txt")), "rU", encoding='utf-8')
